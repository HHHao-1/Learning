...
`String::toString 等价于 s -> s.toString() `
...
| **方法**                              | **描述**                 | **分类** |
| ------------------------------------- | ------------------------ | -------- |
| compare(T, T)                         | 返回负数、0、正数        | 抽象方法 |
| equals (Object)                       | 某个对象是否等于此比较器 | 基类方法 |
| comparing(Function)                   | 自定义比较器排序         | static   |
...
##### count
  ...
  用于统计Stream流中元素的个数`Long count();`，返回值是一个Long类型的整数。
...
> 它作为一个容器，它可能含有某值，或者不包含，用它的目的是尽可能避免NullPointerException。
>
> 它提供的是编译时检查，能极大的降低NPE这种Runtime Exception 对程序的影响，迫使程序员更早的在编码阶段处理空值问题，而不是留到运行时再发现和调试。
...
# collector、collectors、collect的关系**
